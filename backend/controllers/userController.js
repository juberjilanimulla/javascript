...
export const getUsers = asyncHandler(async (req, res) => {
  ...
  return successResponse(res, 200, "Users fetched", rows?.[0] || []);
});
...
export const createUser = asyncHandler(async (req, res) => {
  ...
  return successResponse(res, 201, "User created", { id: result?.[0]?.[0]?.id || null });
});
...
export const updateUser = asyncHandler(async (req, res) => {
  ...
  return successResponse(res, 200, "User updated", { id: Number(id) });
});

export const deleteUser = asyncHandler(async (req, res) => {
  const { id } = req.params;
  await pool.query("CALL sp_delete_user(?)", [id]);
  return successResponse(res, 200, "User deleted", { id: Number(id) });
});